...
| 5. Profiles and Focus | Lead | Native profile filter implemented; exact-head system execution pending | M3–4 | Apple Focus Filter selects any saved Barline Profile by stable identifier; configured app-group store, bounded import, generation-checked workspace/layout/presentation history and rollback, atomic crash-recovery authority envelope, exact-target promotion, original-state recovery, unrelated-state preservation, durable Focus journal, and conservative display reconnect; exact-head signed Focus activation/deactivation remains required |
