...
| 8. OS hardening | Lead | Shelf commit hardening implemented; exact-head full gate pending | M3–7 | Shelf presentation now sizes before ordering, requires two consecutive AppKit/WindowServer confirmations on the active Space, retries once, rolls back stale logical state, and reconciles later order-out. The runtime harness has a real status-item click lane pinned to the exact app PID and window number. Exact-head full/candidate testing, physical scenario matrix, release soak, and macOS 27 remain pending. |
| 9. Distribution readiness | Lead | Prior notarized 1.0.0 installed; replacement candidate validation in progress | M0–8 | Developer ID export, App Group profile validation, notarization, stapling, Gatekeeper, Sparkle signing, checksums, SBOM, and source archive passed for the prior installed candidate; exact-head evidence must be regenerated after the native Focus-profile, Dock preference, and non-hideable ScreenCaptureUI corrections |
