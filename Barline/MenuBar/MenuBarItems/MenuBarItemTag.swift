...
struct MenuBarItemTag: Hashable, CustomStringConvertible {
    ...
    /// A Boolean value that indicates whether the item identified
    /// by this tag is a control item owned by Barline.
    ...
    private init(controlItem identifier: ControlItem.Identifier) {
        self.init(namespace: .barline, title: identifier.rawValue)
    }
}
...
extension MenuBarItemTag {
    ...
    /// An array of tags for items representing Barline's control items.
    static let controlItems = ControlItem.Identifier.allCases.map { $0.tag }

    // MARK: Control Items

    /// The tag for Barline's control item for the "Visible" section.
    static let visibleControlItem = MenuBarItemTag(controlItem: .visible)

    /// The tag for Barline's control item for the "Hidden" section.
    static let hiddenControlItem = MenuBarItemTag(controlItem: .hidden)

    /// The tag for Barline's control item for the "Always-Hidden" section.
    ...
    static let timeMachine = MenuBarItemTag(
...
extension MenuBarItemTag.Namespace {
    /// The namespace for the "Barline" process.
    static let barline = string(Constants.bundleIdentifier)
