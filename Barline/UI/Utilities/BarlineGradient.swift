//
//  BarlineGradient.swift
//  Barline
//

import SwiftUI

// MARK: - BarlineGradient

/// A custom gradient.
struct BarlineGradient: Codable, Hashable {
    ...
    func withAlpha(_ alpha: CGFloat) -> BarlineGradient {
        let newStops = stops.map { $0.withAlpha(alpha) }
        return BarlineGradient(stops: newStops)
    }
...
// MARK: BarlineGradient Static Members
extension BarlineGradient {
    /// The default menu bar tint gradient.
    static let defaultMenuBarTint = BarlineGradient(stops: [
        ColorStop.white(location: 0),
        ColorStop.black(location: 1),
    ])
}

// MARK: - BarlineGradient.ColorStop
...
// MARK: BarlineGradient.ColorStop: Codable
extension BarlineGradient.ColorStop: Codable {
    ...
    init(from decoder: any Decoder) throws {
        ...
        self.color = try container.decode(BarlineColor.self, forKey: .color).cgColor
        self.location = try container.decode(CGFloat.self, forKey: .location)
    }

    func encode(to encoder: any Encoder) throws {
        var container = encoder.container(keyedBy: CodingKeys.self)
        try container.encode(BarlineColor(cgColor: color), forKey: .color)
