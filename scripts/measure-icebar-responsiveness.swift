...
private enum ProbeError: Error, CustomStringConvertible {
    case iceIconNotFound
    case unableToCloseBaseline

    var description: String {
        switch self {
        case .iceIconNotFound:
            "No on-screen Ice.ControlItem.Visible window was found"
        case .unableToCloseBaseline:
            "The Ice Bar could not be closed before measurement"
        }
    }
}
...
private func iceIconCenter() throws -> CGPoint {
    guard
        let icon = windowSnapshots().first(where: {
            $0.windowName == "Ice.ControlItem.Visible" &&
            $0.bounds.width > 0 &&
            $0.bounds.width < 100
        })
    else {
        throw ProbeError.iceIconNotFound
    }
    return CGPoint(x: icon.bounds.midX, y: icon.bounds.midY)
}

private func isIceBarVisible() -> Bool {
    windowSnapshots().contains {
        $0.ownerName == "Ice" && $0.windowName == "Ice Bar"
    }
}
...
do {
    let iconPoint = try iceIconCenter()
